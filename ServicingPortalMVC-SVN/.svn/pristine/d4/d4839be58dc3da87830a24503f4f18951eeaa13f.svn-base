...
public class ManagerSession {
	public static void userCustomer(CustomerAccount user, HttpServletRequest request) {
		HttpSession session = request.getSession();
		session.setAttribute(CommonConstrants.USER_SESSION, user);
	}
	public static CustomerAccount userCustomer(HttpServletRequest request) {
		CustomerAccount res = null;
		HttpSession session = request.getSession();
		res = (CustomerAccount) session.getAttribute(CommonConstrants.USER_SESSION);
		return res;
	}
